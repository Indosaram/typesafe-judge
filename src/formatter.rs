...
pub fn print_compact_summary(res: &SystemOneResponse, elapsed_ms: u128) {
    ...
    for key in keys {
        ...
        match answer {
            Answer::Choice(choice) => {
                let win_p = choice.probabilities.get(&choice.choice).copied().unwrap_or(0.0);
                let mut others: Vec<String> = choice
                    .probabilities
                    .iter()
                    .filter(|(k, _)| *k != &choice.choice)
            ...
            Answer::Noul(noul) => {
                ...
                let label = if p >= 0.70 {
                    ...
                    "LEANING_YES".bright_yellow().bold()
                } else if p >= 0.30 {
                    "LEANING_NO".bright_yellow().bold()
                } else {
                    "NO".bright_red().bold()
                };
            ...
            Answer::Score(score) => {
                let top_desc = score
                    .legend
                    .get(&format!("{}", score.score.round() as u64))
                    .cloned()
                    .unwrap_or_default();

                let desc_part = if top_desc.is_empty() {
                    String::new()
                } else {
                    format!(" ({})", top_desc)
                };
