...
pub fn render_confidence(conf: f64) -> ColoredString {
    ...
    if conf >= 0.80 {
        format!("● High Confidence ({})", text).bright_green().bold()
    } else if conf >= 0.50 {
        format!("▲ Moderate Confidence ({})", text).bright_yellow().bold()
    } else {
        format!("■ Low Confidence ({})", text).bright_red().bold()
    }
}

pub fn print_response_summary(res: &SystemOneResponse, elapsed_ms: u128) {
    println!();
    println!(
        "{} {} {} {}",
        "───".dimmed(),
